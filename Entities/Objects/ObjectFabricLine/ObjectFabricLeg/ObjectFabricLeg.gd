...
#	draw_circle($RayCast2D.get_collision_point() - get_parent().global_position - position, 10.0, Color(1,0,0,1))
...
#	print("current vel: " + str(current_origin_velocity) + "    max vel: " + str(max_origin_velocity) + "    lerp val: " + str(current_origin_velocity.x / max_origin_velocity.x))
	...
	if current_step_position.distance_to(points[points.size()-1] + get_parent().global_position + position) > min_acceptable_distance: # or current_step_position.distance_to($RayCast2D.get_collision_point()) > step_distance:
		print("current pos: " + str(current_step_position) + "    endpoint pos: " + str(points[points.size()-1] + get_parent().global_position + position))
